...
PredatorLite is an independent, unofficial interoperability project. The repository contains factual interface information needed to communicate with Acer components installed on hardware owned and tested by project contributors.
...
Values are validated against an Acer Predator PHN16-71 with BIOS V1.20. Hardware writes are enabled only for that exact target and only after capability probing. New mappings require capture evidence, failure testing, read-back verification, and recovery coverage.
...
The repository does not distribute Acer source code, decompiled code, executables, drivers, firmware, ROM images, or vendor artwork. The ignored local `PreySense/` directory is not part of the project or its Git history and must not be used as a source for contributed code or assets.

Contributors must submit independently authored implementations and enough protocol evidence to review each new hardware-write path. Do not attach proprietary binaries, firmware, serial numbers, or unredacted diagnostics to issues or pull requests.
