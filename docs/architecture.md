...
The application talks to hardware only through `IPredatorPlatform`, which keeps protocol code and UI state separate. Hardware writes are authorized by the explicit profile catalog in `PredatorLite.Platform.Windows`; unknown identities remain read-only. All Windows projects target Windows 11 24H2 (`10.0.26100+`) x64 on .NET 10. `PredatorLite.App` is an unpackaged, framework-dependent Windows App SDK 2.3.1 application; the release script publishes RID-specific ReadyToRun assemblies without embedding the .NET or Windows App Runtime.
...
5. After the exact hardware profile, power state and a usable control backend succeed, restore the saved non-Eco mode, or Eco on battery when that automation is enabled. A freshly verified matching mode only reapplies the Windows power overlay and sends no Acer write.
...
A target whose profile is known but whose control backend is still starting receives read-only retries within one cancellation-aware ten-second deadline. Failure leaves the app usable and read-only; a backend first discovered by the deferred probe gets at most one pending startup restore attempt.
...
1. Resolve the exact hardware profile and confirm that the model, BIOS and requested control are authorized.
2. Serialize commands so AcerService/WMI operations cannot overlap.
3. Send only the command associated with the selected profile-authorized UI control.
...
Auto fan mode does not require a watchdog. Before Max or Custom is applied, the app launches `PredatorLite.FanGuard.exe`, completes a current-user-only named-pipe handshake and registers an active lease at the platform boundary.
...
FanGuard first resolves an authorized profile, then uses AcerService and falls back to the documented Acer WMI method. `PredatorPlatform` tracks the active lease and fan ownership, so a lost guard cannot be followed by another Custom write and closing PredatorLite does not alter a Max or Custom state that existed before this process started.
