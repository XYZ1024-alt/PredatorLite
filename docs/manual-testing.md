...
5. From a clean ordinary-user profile with the documented runtimes installed, install without elevation. Verify the default location is `%LocalAppData%\Programs\PredatorLite`, the Start menu entry works, and the optional desktop shortcut follows the selected task.
