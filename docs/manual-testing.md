...
Run this checklist on Windows 11 24H2 (build 26100+) x64. The fully validated reference device is Acer Predator PHN16-71 with BIOS V1.20. On any other model or BIOS, hardware writes must still open when an Acer backend probe succeeds, and remain disabled only when no backend is available.
...
5. Delay or stop AcerService on the current profile machine. Verify startup probes stop at the ten-second deadline and do not loop hardware writes. If the deferred probe first discovers the backend, it may perform only one pending restore.
6. On an unknown model or BIOS, verify hardware controls open after a successful backend probe. A denied Acer WMI mode read must not make `CanWriteHardware` true when no Acer backend is available.
...
14. Stop AcerService only for a controlled failure case. With WMI available, resume restoration must use its verified fallback. Without WMI, the log must show the ten-second read deadline and one failed outcome without changing `LastAcMode`. Restart AcerService immediately, then repeat on an unknown model or BIOS to confirm restoration requires an available backend.
...
4. Manually dispatch the `release` workflow. From `main`, Beta must require a positive iteration, reject `confirm_public=true`, and create a Draft Pre-release visible only to users with push access; RC must require a positive iteration plus `confirm_public=true` and create a public Pre-release; Stable must reject an iteration, require `confirm_public=true`, and create a normal public Release. From a non-main branch, Beta must create the same maintainer-only Draft Pre-release from the selected branch revision, while RC and Stable must fail before packaging. Every channel must reject an existing complete version instead of overwriting it. Do not grant public-repository push access solely for Beta downloads; use a private repository or private storage for broader internal QA.
...
1. Install the Visual Studio MSIX Packaging Tools/WAP targets, then run `build\build-store-package.ps1 -Configuration Release -Version 1.1.1 -Mode StoreUpload`. Confirm the command fails before promotion if the tools are absent, the version differs from `Directory.Build.props`, or any Store MSBuild property is overridden.
2. Run `build\test-store-package.ps1 -PackagePath publish\store\PredatorLite-Store-1.1.1-win-x64.msixupload -ExpectedVersion 1.1.1.0`. Confirm the upload contains exactly one x64 MSIX and no ARM/x86 bundle, symbols, source, XML documentation, or loose `.cer`/`.pfx` file.
3. Unpack the validated MSIX and confirm the Partner Center identity name, publisher, publisher display name, `1.1.1.0` version, x64 architecture, `runFullTrust`, `PredatorLiteStartup`, seven package assets, and `en-US`/`zh-CN` resources. Confirm the package root contains `PredatorLite.pri`, `App.xbf`, `MainWindow.xbf`, and `OsdWindow.xbf`, and that all XBF files beneath `Resources` and `Views` are present. Confirm `allowElevation` and every `windows.fullTrustProcess` helper extension are absent; `PredatorLite.exe` and `PredatorLite.FanGuard.exe` must each exist exactly once at package root, and no `PredatorLite.ElevatedHelper.*` payload may exist anywhere in the package.
